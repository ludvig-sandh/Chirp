...
BaseFilter::BaseFilter(Audio::Core::Frequency cutoff, float Q) noexcept : m_cutoff(cutoff), m_Q(Q) {}

void BaseFilter::ProcessFrame(Audio::Engine::AudioFrame& output) noexcept {
    output.left = m_leftFilter.Step(output.left);
    output.right = m_rightFilter.Step(output.right);
}

void BaseFilter::SetCutoff(Audio::Core::Frequency cutoff) noexcept {
...
void BaseFilter::SetCutoffAndPeaking(Audio::Core::Frequency cutoff, float Q) noexcept {
    m_cutoff.SetFrequency(std::clamp(cutoff.GetAbsolute(), MIN_CUTOFF, MAX_CUTOFF));
    m_Q = std::clamp(Q, MIN_Q, MAX_Q);
    ComputeAndApplyCoefficients();
}
